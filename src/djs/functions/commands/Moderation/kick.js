...
async function kick(target, reason, type, trigger) {
  ...
  if (type === "slash") {
    ...
    if (
      trigger.member.roles.highest.position <= target.roles.highest.position
    ) {
    ...
    // check if the trigger user has a lower role than the target
    if (
      trigger.member.roles.highest.position <= member.roles.highest.position
    ) {
      // if the trigger user has a lower role than the target, send an error embed
      const errEmbed = createEmbed({
        title: `Unable to Kick`,
