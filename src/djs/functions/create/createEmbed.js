...
function createEmbed(obj) {
  ...
  if ((!obj.title && !obj.description && !obj.image && (!obj?.fields || obj?.fields?.length === 0)) || (obj.title === null && obj.description === null && obj.image === null && (!obj?.fields || obj?.fields?.length === 0))) {
    // If not, log an error
    console.log(obj)
    if (!obj.thumbnail) {
      try {
